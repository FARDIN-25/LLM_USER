...
def apply_refactor():
    ...
    print("\n>>> RUNNING PHASE: 2: CLEAN EXISTING TEXT DATA (Regex normalization)")
    ...
    # PHASE 3: VERIFY
    run_phase(engine, "3: VERIFY (Sample)", "SELECT chunk_text, clean_text FROM book_chunks LIMIT 5;")
    
    # PHASE 4: NOISE REMOVAL
    run_phase(engine, "4: NOISE REMOVAL (Destructive)", """
        DELETE FROM book_chunks 
        WHERE lower(clean_text) LIKE '%preface%' 
           OR lower(clean_text) LIKE '%appendix%' 
           OR lower(clean_text) LIKE '%form gst%' 
           OR lower(clean_text) LIKE '%section%' 
